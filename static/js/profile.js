        ...
        .map(
          (s) =>
            `<div class="sport-item">
              <div>
                <strong>${escapeHtml(s.sport_name)}</strong>
                <small>현재 실력 ${escapeHtml(skillLevelLabel(s.skill_level))}</small>
              </div>
              <div class="actions">
                <select data-level="${s.sport_id}">
                  ${SKILL_LEVELS.map(
                    (v) =>
                      `<option value="${v}" ${
                        v === s.skill_level ? 'selected' : ''
                      }>${escapeHtml(skillLevelLabel(v))}</option>`,
                  ).join('')}
                </select>
                <button
                  class="btn sm"
                  data-update-sport="${s.sport_id}"
                >
                  수정
                </button>
                <button
                  class="btn sm danger"
                  data-delete-sport="${s.sport_id}"
                  data-name="${escapeHtml(s.sport_name)}"
                >
                  삭제
                </button>
              </div>
...
document.getElementById('imageForm').addEventListener('submit', async (e) => {
  ...
  if (!file) {
    return showToast('이미지 파일을 선택하세요.');
  }

  // 브라우저에서도 5MB 초과 파일을 먼저 차단
  if (file.size > MAX_PROFILE_IMAGE_SIZE) {
    return showToast('프로필 이미지는 5MB 이하만 업로드할 수 있습니다.');
  }
